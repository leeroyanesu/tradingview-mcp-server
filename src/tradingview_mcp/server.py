...
async def get_chart_snapshot(
    symbol: str, interval: str = "D", width: int = 1200, height: int = 600, theme: str = "dark"
) -> Optional[bytes]:
    """
    ...
    try:
        ...
        invalid_indicators = ["Invalid symbol", "Symbol not found", "This symbol is not available"]
...
async def get_mt5_ohlc(
    symbol: str, timeframe_str: str, count: int = 150
...
async def render_lightweight_chart(
    data: List[Dict], symbol: str, theme: str = "dark", width: int = 1200, height: int = 600
) -> Optional[bytes]:
...
async def call_tool(name: str, arguments: dict) -> list[TextContent | ImageContent]:
    ...
    if not session_id or not session_id_sign:
        ...
        if not image_data:
            ...
            if ohlc_data:
                image_data = await render_lightweight_chart(
